...
class DCGRUDecoder(nn.Module):
    ...
    def forward(
        self, inputs, supports, initial_hidden_state, teacher_forcing_ratio=0.5
    ):
        seq_length = inputs.shape[0]
        batch_size = inputs.shape[1]
        inputs = torch.reshape(inputs, (seq_length, batch_size, -1))
        ...
        outputs = torch.zeros(
            seq_length, batch_size, self.node_num * self.output_dim
        ).to(self.device)
    ...
    def forward(self, inputs, supports, state):
        ...
        if self._num_proj is not None:
            ...
            output = torch.reshape(
                self.project(output), shape=(batch_size, self.output_size)
            )
...
class DiffusionGraphConv(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.FloatTensor(size=(input_size * self.num_matrices, output_dim))
        )
