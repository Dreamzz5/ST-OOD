...
class DSTAGNN(BaseModel):
    ...
    def __init__(
        ...
        **args
        ...
        self.final_conv = nn.Conv2d(
            int((self.seq_len / time_stride) * nb_block),
            128,
    ...
    def forward(self, x, label=None):  # (b, t, n, f)
        ...
        output = self.final_conv(final_x.permute(0, 3, 1, 2))[:, :, :, -1].permute(
            0, 2, 1
        )
...
class DSTAGNN_block(nn.Module):
    def __init__(
        ...
        self.cheb_conv_SAt = cheb_conv_withSAt(
            K, cheb_polynomials, in_channels, nb_chev_filter, node_num
        )
        ...
        self.residual_conv = nn.Conv2d(
            in_channels, nb_time_filter, kernel_size=(1, 1), stride=(1, time_stride)
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask, res_att):
        ...
        Q = (
            self.W_Q(input_Q)
            .view(bs, self.num_of_d, -1, self.n_head, self.d_k)
            .transpose(2, 3)
        )
        K = (
            self.W_K(input_K)
        ...
        V = (
            self.W_V(input_V)
            .view(bs, self.num_of_d, -1, self.n_head, self.d_v)
            .transpose(2, 3)
        )
        ...
        context, res_attn = ScaledDotProductAttention(self.d_k)(
            Q, K, V, attn_mask, res_att
        )
        context = context.transpose(2, 3).reshape(
            bs, self.num_of_d, -1, self.n_head * self.d_v
        )
...
class cheb_conv_withSAt(nn.Module):
    def __init__(self, K, cheb_polynomials, in_channels, out_channels, node_num):
        ...
        self.mask = nn.ParameterList(
            [
                nn.Parameter(torch.FloatTensor(node_num, node_num).to(self.device))
                for _ in range(K)
            ]
        )
    ...
    def forward(self, x, bs):
        if self.Etype == "T":
            ...
            pos = (
                pos.unsqueeze(0)
                .unsqueeze(0)
                .expand(bs, self.num_of_features, self.nb_seq)
            )
