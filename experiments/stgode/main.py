...
def main():
    ...
    if os.path.isfile(os.path.join(log_dir, "se_matrix.npy")):
        se_matrix = np.load(os.path.join(log_dir, "se_matrix.npy"))
    else:
        se_matrix = construct_se_matrix(data_path, args)
        np.save(os.path.join(log_dir, "se_matrix.npy"), se_matrix)
...
def construct_se_matrix(data_path, args):
    ...
    idx = np.load(os.path.join(data_path, args.years, "idx_train.npy"))
    data = ptr["data"][idx][..., 0]
