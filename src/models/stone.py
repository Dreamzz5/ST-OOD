...
class STONE(BaseModel):
    def __init__(
        ...
        **args,
        ...
        self.gatefusion = GatedFusionBlock(
            sem_input_dim=sem_output_dim,
    ...
    def forward(self, x, sem, adj=None, tadj=None, label=None):
        ...
        x, sem, x_adj, sem_adj = self.staggblocks(
            x.permute(0, 3, 1, 2).squeeze(-1), sem
        )
...
class STBlock(nn.Module):
    def __init__(
        ...
        for j in range(len(TBlocks)):
            dilation = 1
            self.t_mlp.append(
                Dilation_Gated_TemporalConvLayer(
                    Kt,
                    TBlocks[j][0],
                    TBlocks[j][-1],
                    dilation,
        ...
        self.t_mlp.append(
            Dilation_Gated_TemporalConvLayer(
                2, TBlocks[j][0], TBlocks[j][-1], dilation, node_num_ob + node_num_un
            )
        )
        self.relu = nn.ReLU()
...
class STAggBlock(nn.Module):
    def __init__(
        ...
        self.t_adpadj = AdaptiveInteraction(
            x_input_dim, adp_s_dim, node_num_ob + node_num_un
        )
        self.s_adpadj = AdaptiveInteraction(
            sem_input_dim, adp_t_dim, node_num_ob + node_num_un
        )
...
class AdaptiveInteraction(nn.Module):
    ...
    def forward(self, input):
        if True:
            ...
            E_out = torch.einsum("bid,bjd->bij", E_out1, E_out2) / math.sqrt(
                self.output_dim
            )
            E_in = torch.einsum("bid,bjd->bij", E_in1, E_in2) / math.sqrt(
                self.output_dim
            )
            ...
            if len(input.shape) == 2:
                adp_adj = torch.einsum("bik, bjk -> bij", E_out3, E_in3) / math.sqrt(
                    self.output_dim
                )
            else:
                adp_adj = torch.einsum("bik, bjk -> bij", E_out3, E_in3) / math.sqrt(
